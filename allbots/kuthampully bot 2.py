...
def load_documents_pdf():
    loader = DirectoryLoader('datas/kuthampully bot 2/', glob="*.pdf", loader_cls=PyPDFLoader)
...
def create_llms_model():
    llm = HuggingFaceHub(repo_id="mistralai/Mistral-7B-Instruct-v0.2", model_kwargs={"temperature":0.5, "max_length":1000})
    return llm
...
if st.session_state["authentication_status"] and st.session_state["username"]=="vishnu":
    ...
    st.title("kuthampully bot 2")
    st.markdown('<style>h1{color: orange; text-align: center;}</style>', unsafe_allow_html=True)
    st.subheader('ask about kuthampully')
