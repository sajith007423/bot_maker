...
def load_documents_pdf():
    loader = DirectoryLoader('datas/gemstone bot/', glob="*.pdf", loader_cls=PyPDFLoader)
...
if st.session_state["authentication_status"] and st.session_state["username"]=="sajith":
    drawing_name = st.session_state["name"]
    st.subheader(f"created by {drawing_name}")
    st.title("gemstone bot")
    st.markdown('<style>h1{color: orange; text-align: center;}</style>', unsafe_allow_html=True)
    st.subheader('ask about gemstones')
