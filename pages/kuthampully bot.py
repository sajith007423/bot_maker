...
def load_documents_pdf():
    loader = DirectoryLoader('datas/kuthampully bot/', glob="*.pdf", loader_cls=PyPDFLoader)
...
def create_llms_model():
    llm = HuggingFaceHub(repo_id="mistralai/Mixtral-8x7B-Instruct-v0.1", model_kwargs={"temperature":0.5, "max_length":1000})
    return llm


if st.session_state["authentication_status"] and st.session_state["username"]=="vishnu":
    drawing_name = st.session_state["name"]
    st.subheader(f"created by {drawing_name}")
    st.title("kuthampully bot")
    st.markdown('<style>h1{color: orange; text-align: center;}</style>', unsafe_allow_html=True)
    st.subheader('discuss about kuthampully')
